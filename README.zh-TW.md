...
cd slim && python3 -m unittest discover -s . -p 'test_*.py'    # 105 tests，只用標準庫
```

## 為 CPE 建置

```bash
make -C slim/cpp t830           # 動態連結，142,352 bytes；裝置上需要 libstdc++/libgcc
make -C slim/cpp t830-static    # 719,144 bytes、NEEDED 0、已 strip；裝置上不需安裝任何東西
```
...
7B：答案會弱一些；它能吐出 prompt 裡展示的 tool JSON 格式，但不穩定，所以要用斜線指令驅動動作
（`/rag`、`/read`、`/write`、`/mqtt`、`/history`）才靠得住。兩者可以依每次呼叫並存，`--fallback-url` 還能做出
...
| 檢查項目 | CPE 上的結果 |
| --- | --- |
| artifact 完整性 | `sha256 1e365e37...` 在主機與裝置兩邊一致（`NEEDED 0`、已 strip） |
