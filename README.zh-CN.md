...
```bash
make -C slim/cpp t830           # 动态链接，142,352 bytes；装置上需要 libstdc++/libgcc
make -C slim/cpp t830-static    # 719,144 bytes、NEEDED 0、已 strip；装置上无需安装任何东西
```
...
| 检查项 | CPE 上的结果 |
| --- | --- |
| artifact 完整性 | `sha256 1e365e37...` 在主机与装置两边一致（`NEEDED 0`、已 strip） |
